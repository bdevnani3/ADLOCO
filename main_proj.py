...
#LogReg model config
parser.add_argument("--c", default=10000, type=float)
...
def get_log_and_tf_dir(cfg):
    ...
    cfg = cfg.split("configs_proj/")[1]
    cfg = "_".join(cfg.split("/"))
    if len(args.expt_str) > 0:
        log_cfg = (
            f"/nethome/bdevnani3/flash1/long_tail_lang/results_proj/config_"
            + cfg
            + "/"
            + args.expt_str
            + "/"
...
if not test_mode:
    ...
    training_model.eval(phase=test_split)
