...
class model:
    def __init__(self, config, data, test=False):
        ...
        self.logit_scale = nn.Parameter(torch.log(10 * torch.ones((1, 1, 1)))).float().cuda()
        ...
        if check_config(self.training_opt, "proj_path"):
            self.proj = (
                torch.tensor(np.load(self.training_opt["proj_path"])).cuda()
            )
            print(
    ...
    def init_models(self, optimizer=True):
        ...
        self.fusion = nn.Linear(in_dim, out_dim, bias=False)

        if check_config(self.training_opt, "init_with_text_prototypes"):
            if self.training_opt["init_with_text_prototypes"]:
                print("Initializing fusion layer with text prototypes.")
                classnames = CLASSES
                ...
                self.fusion.weight = torch.nn.Parameter(zeroshot_weights)
                # import pdb; pdb.set_trace()

        self.fusion = torch.nn.DataParallel(self.fusion.double()).cuda()

        if "text_proj" in self.variation:
            self.text_proj = torch.nn.DataParallel(nn.Linear(1024, 1024, bias=False).float()).cuda()
        ...
        if "text_proj" in self.variation:
            optim_params_text_proj = self.model_opt["text_proj"]["optim_params"]
            self.model_optim_params_list.append(
                {
                    "params": self.text_proj.parameters(),
                    "lr": optim_params_text_proj["lr"],
                    "momentum": optim_params_text_proj["momentum"],
                    "weight_decay": optim_params_text_proj["weight_decay"],
                }
            )
            self.model_optim_params_list_LBFGS.extend(self.text_proj.parameters())
        
        self.bn = nn.BatchNorm1d(1000).cuda()
        self.model_optim_params_list_LBFGS.extend(self.bn.parameters())
    ...
    def batch_forward(self, inputs, labels, phase="train", paths=None):
        ...
        elif variation == "image_plus_text_proj":
            self.logits = image_plus_text_proj(self, inputs, labels, phase=phase)
        elif variation == "all_text_with_transformation":
            self.logits = all_text_with_transformation(
                self, inputs, labels, phase=phase, paths=paths, proj=self.proj
            )
    ...
    def batch_loss(self, labels):
        ...
        if "ClipLoss" in self.criterions.keys():
            self.loss_clip = self.criterions["ClipLoss"](self.image_embs.cuda(), self.projected_text.cuda(), self.logit_scale, self.logits)
            self.loss_clip *= self.criterion_weights["ClipLoss"]
            self.loss += self.loss_clip

        if "OPLoss" in self.criterions.keys():
            self.loss_op = self.criterions["OPLoss"](self.proj_weights)
            self.loss_op *= self.criterion_weights["OPLoss"]
    ...
    def minibatch_training_results(self, step, epoch, preds, labels):
        ...
        if step % self.training_opt["display_step"] == 0:
            ...
            minibatch_loss_clip = (
                self.loss_clip.item() if "ClipLoss" in self.criterions else None
            )
            minibatch_loss_op = (
                self.loss_op.item() if "OPLoss" in self.criterions else None
            )
            ...
            print_str = [
                ...
                "Minibatch_loss_clip: %.3f" % (minibatch_loss_clip)
                if minibatch_loss_clip
                else "",
                "Minibatch_loss_op: %.3f" % (minibatch_loss_op)
                if minibatch_loss_op
                else "",
    ...
    def train_epoch(self, epoch):
        ...
        if self.eval_acc_mic_top1 > self.best_acc:
            ...
            if "text_proj" in self.variation:
                self.best_model_weights["text_proj"] = copy.deepcopy(self.text_proj.state_dict())
    ...
    def load_model(self, model_dir=None):
        ...
        if "text_proj" in self.variation:
            self.text_proj.load_state_dict(model_state["text_proj"])
    ...
    def save_latest(self, epoch):
        ...
        if "text_proj" in self.variation:
            model_weights["text_proj"] = copy.deepcopy(self.text_proj.state_dict())
